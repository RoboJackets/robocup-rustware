...
pub struct MotionControl {
    ...
    // Current Target velocity <v_x(t), v_y(t), w_(t)>
    //                          ((m/s), (m/s),  (m/s))
    current_target_velocity: Vector3<f32>,
...
impl MotionControl {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            current_target_velocity: Vector3::zeros(),
    ...
    /// It seems that the FPGA doesn't like ramping between values that are far apart.
    /// Therefore, trapazoid_control_update attempts to ramp between setpoints linearly
    /// to, hopefully, force the fpga to respond in a timely manner.
    ///
    /// The basic principle of this update is that when a new setpoint is detected, the
    /// next 100 motion control updates will be a linear interpolation between the last
    /// setpoint and the next setpoint.
    ///
    /// Parameters:
    ...
    ///     delta: elapsed time from the last motion control update (us)
    ///
    /// Returns:
    ///     (motor_1, motor_2, motor_3, motor_4) (m/s)
    pub fn trapazoid_control_update(
        &mut self,
        ...
        // Detect if we are changing directions
        if target_velocity != self.last_target_velocity {
            // We are now moving
            self.timesteps = 0;
            self.last_target_velocity = self.current_target_velocity;
            self.current_target_velocity = target_velocity;
        }

        if self.timesteps < 100 {
            // Linearly interpolate between the last target and the current target
            let delta = (self.current_target_velocity - self.last_target_velocity)
                .map(|v| v * (self.timesteps as f32) / 100.0);
            self.bot_to_wheel * (self.last_target_velocity + delta) + self.wheel_correction
        } else {
