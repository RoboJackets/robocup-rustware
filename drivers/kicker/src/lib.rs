//!
//! Driver for the Robocup Kicker Board
//! 
...
pub struct Kicker<SPI, CSN, SPIE, GPIOE> where 
    SPI: ConfigurableSpi<Error=SPIE>,
...
impl<SPI, CSN, RESET, SPIE, GPIOE> Kicker<SPI, CSN, RESET, SPIE, GPIOE> where 
    SPI: ConfigurableSpi<u8, Error=SPIE>,
    CSN: OutputPin<Error=GPIOE>,
    ...
    pub fn set_from_control_message(&mut self, control_message: &ControlMessage) {
        match control_message.shoot_mode {
            true => self.kick_type = KickType::Chip,
            false => self.kick_type = KickType::Kick,
        }

        match *control_message.trigger_mode {
            0b01 => self.kick_trigger = KickTrigger::Breakbeam,
            0b10 => self.kick_trigger = KickTrigger::Immediate,
    ...
    ///         1 Chip
    ///         0 Kick
    pub fn service(&mut self, spi: &mut SPI, delay: &mut impl DelayMs) -> Result<(), KickerError<SPIE, GPIOE>> {
        ...
        spi.transfer(&mut command).map_err(KickError::SpiError)?;
        // Delay at least 10us
        delay.delay_us(50u32);
        self.csn.set_high().map_err(KickerError::SpiError)?;

        // Decode current_voltage and ball_sense from the response from kicker
        self.current_voltage = (self.command[0] & VOLTAGE_MASK) * VOLTAGE_SCALAR;
        self.ball_sensed = (self.command[0] & BREAKBEAM_TRIPPED) != 0;
