...
use embedded_graphics::{
    mono_font::{ascii::{FONT_7X13, FONT_6X9}, MonoTextStyle},
...
impl<'a> ErrorScreen<'a> {
    ...
    pub fn wrap(&self, text: &str) -> Vec<String> {
        ...
        let iters = text.len() / 20;
        if iters == 0 {
            strs.push(text.into());
            return strs;
        } else {
            for i in 0..iters {
                let temp = text[(i * 20)..((i + 1) * 20)].into();
                strs.push(temp);
            }
            strs.push(text[(iters * 20)..].into());
            return strs;
        }
    }
}
...
impl<'a> Drawable for ErrorScreen<'a> {
    ...
    fn draw<D>(&self, target: &mut D) -> Result<Self::Output, D::Error>
        ...
        for (i, line) in wrapped.iter().enumerate() {
            Text::with_text_style(line, Point::new(1, (20 + 8 * i) as i32), char_style, text_style)
            .draw(target)?;
        }
        Ok(())
    }
