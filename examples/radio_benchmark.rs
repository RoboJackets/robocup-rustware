...
mod app {
    ...
    #[task(binds = GPIO1_COMBINED_0_15, local = [gpio1, rx_int], shared = [radio, blocking_delay])]
    fn receive_data(ctx: receive_data::Context) {
        log::info!("Interrupt Triggered");

        ctx.local.gpio1.set_interrupt(ctx.local.rx_int, None);
        ctx.local.rx_int.clear_triggered();

        (ctx.shared.radio, ctx.shared.blocking_delay).lock(|radio, delay| {
            let mut start = 0;
            ...
            ctx.local.rx_int.clear_triggered();
            ctx.local.gpio1.set_interrupt(ctx.local.rx_int, Some(Trigger::RisingEdge));
        });
    }
}