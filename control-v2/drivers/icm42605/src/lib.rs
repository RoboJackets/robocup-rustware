...
pub fn reading_to_accel(high: u8, low: u8) -> f32 {
    let value = i16::from_be_bytes([high, low]);
    ((value as f32) * LSB_TO_G) * 9.81
}

#[derive(Debug)]
/// An error from operating the IMU
pub enum ImuError<E: Format> {
...
impl<I2C: I2c<Error=E>, E: Format> Imu<I2C> {
    ...
    pub fn init(&mut self, delay: &mut impl DelayNs) -> Result<(), ImuError<E>> {
    ...
    /// Read the z-axis gyro (in degrees-per-second)
    ...
    /// Read the x-axis gyro (in degrees-per-second)
    pub fn gyro_x(&mut self) -> Result<f32, ImuError<E>> {
        if !self.initialized {
            return Err(ImuError::Uninitialized);
        }
    ...
    /// Read the y-axis gyro (in degrees-per-second)
    pub fn gyro_y(&mut self) -> Result<f32, ImuError<E>> {
        if !self.initialized {
            return Err(ImuError::Uninitialized);
        }
    ...
    /// Read the x-axis acceleration (in m/s^2)
    ...
    /// Read the y-axis acceleration (in m/s^2)
    ...
    /// Read the z-axis acceleration (in m/s^2)
    pub fn accel_z(&mut self) -> Result<f32, ImuError<E>> {
        if !self.initialized {
            return Err(ImuError::Uninitialized);
        }
