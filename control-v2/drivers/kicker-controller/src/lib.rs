...
pub struct Kicker<RESET, SPI> {
    /// The current state of the kicker
    pub state: KickerState,
    /// The reset pin of the kicker (needs to be held high)
    reset: RESET,
    /// The spi connected to the kicker
    spi: SPI,
}

impl<RESET: OutputPin<Error = GPIOE>, SPI: SpiDevice<Error=SPIE>, GPIOE: Debug, SPIE: Debug>
    Kicker<RESET, SPI>
{
    /// Create a new kicker control driver
    pub fn new(spi: SPI, mut reset: RESET) -> Self {
    ...
    pub fn service(
        ...
        self.spi.transfer_in_place(&mut buffer).map_err(KickerServiceError::Spi)?;
